...
#tool.example(continuation: true)[
  ...
  مثلاً با رسم یک خط افقی فرضی از دومین انشعاب درخت، داده ها به ۳ خوشه تقسیم می بندی می شوند (مثل خوشه های انگور #emoji.face.tongue).
]
...
#tool.definition()[
  + روش اول:
    ...
    $
    میانگین و انحراف معیار داده های بالا به صورت زیر است (شاید درست حساب نشده باشن. اگر درست نبود هم فرض کنید درسته. فقط یه مثاله #emoji.face.teeth):
    $
      mu = 20 \
      sigma = 5
    $
  ...
  + روش دوم:
    ابتدا چهارک های داده ها را حساب می کنیم.
    در این روش، انواع چهارک ها به صورت زیر تعریف می شوند.
    دقت شود که چهارک های این روش با چهارک هایی که در آمار وجود دارند، کمی متفاوت است (به علامت کف دقت شود):
    $
      \
    ...
    $
    برای محاسبه چهارک های مورد نیاز، داده ها را مرتب می کنیم:
    $
    ...
    در ادامه، IQR یا همان فاصله چهارکی را به صورت زیر حساب می کنیم:
    $
